...
- **BGM**: `resources/うさぎのカフェテリア.mp3`（[DOVA-SYNDROME](https://dova-s.jp/) 提供のフリーBGM）があればそれを再生します。同サイトの規約により音源の再配布が禁止されているためリポジトリには含めておらず、ファイルがない環境（公開版を含む）では、同曲の雰囲気に寄せた Web Audio 合成のオリジナルBGM（カフェ風ボサノバ調）を自動的に再生します。♪ ボタンでオン/オフでき、設定は保存されます
